...
import beamngpy.sensors as bsens

from .processing import NGVehicleSense, NGSight, NGTermination


class AgentNGSimulation:
    ...
    def __init__(
        self,
        scenario:bngpy.Scenario = None,
        start_pos:tuple = (0,0,0),
        start_rot:tuple = (0,0,0),
        sight_resolution:tuple[int, int] = (256, 32)
    ) -> None:
        import yaml
        from beamngpy import BeamNGpy, Vehicle, Scenario, angle_to_quat, logging
        from ..io.yaml_read import MnvrConfig
        
        self.cfg = MnvrConfig()
        
        self.bng = self.cfg.get_prebaked_beamng()
        self.sight_res = sight_resolution
        try:
            self.bng.open(launch=False)
            self.bng.logger.setLevel(logging.logging.WARNING)
        except:
    ...
    def run(
        ...
        def update(step_n:int) -> tuple[dict, dict]:
            ...
            try:
                ...
                flat_sight_tensor = sight.to_tensor()
                sense_tensor = v_sense.to_tensor()
                
                if shape_debug:
                    print("Sight shape: ", flat_sight_tensor.shape)
                    print("Sense shape: ", sense_tensor.shape)
                joined_input_data = tf.concat([flat_sight_tensor, sense_tensor], 0)
                model_input_data = tf.expand_dims(joined_input_data, 0)

                if shape_debug:
                    print("Shape into agent: ", model_input_data.shape)
                
                control = agent(model_input_data)
                ctl_bare = tf.squeeze(control)
                steer_ctl = float(ctl_bare[0])
                power_ctl = float(ctl_bare[1])
                ebrake_ctl = float(ctl_bare[2])
                ...
                if step_n % sample_step_size == 0: # Check if it's a sample round.
                    ...
                    if len(transition_samples) > 4 and float(v_sense.airspeed) < 0.18: # Skip if too slow,excp first 5
                        print("Probably stuck, skipping")
                    else:
                        print("INFO: Saving sample..")
                        transition_samples.append((model_input_data, control))

                vehicle.control(
                    steering=steer_ctl,
                    throttle=power_ctl,
                    brake=-power_ctl, # Invert throttle axis for brake control, thanks sigmoid function :)
                    parkingbrake=ebrake_ctl, # 0-multiplied in early training
                    gear=1 # Keep in first gear, at least in early training
                )
        ...
        for current_step in range(limit_steps):
            ...
            if float(damage_data['damage']) > 500:
                break
    ...
    def _vehicle_init(self) -> bngpy.Vehicle:
    ...
    def _scenario_preflight(self, vehicle:bngpy.Vehicle) -> None:
        scenario = self.target_scenario
        try:
            scenario.add_vehicle(vehicle, pos=self.start_pos, rot_quat=bngpy.angle_to_quat(self.start_rot))
        except:
    ...
    def _sensors_preflight(
        self,
        bng:bngpy.BeamNGpy,
        vehicle:bngpy.Vehicle
    ) -> tuple[bsens.AdvancedIMU, bsens.PowertrainSensor, bsens.Camera]:
        ...
        camera = Camera(
            ...
            pos=(0, -0.25, 1.5),
            dir=(0, -1, 0),
            resolution=(self.sight_res[0], self.sight_res[1]),
            field_of_view_y=24.,
